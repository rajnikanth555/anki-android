...
public class ActivityExportingDelegate implements ExportDialogListener, ExportCompleteDialogListener {
     ...
     */
    public ActivityExportingDelegate(AnkiActivity activity, Collection collection) {
        mActivity = activity;
        this.mCollection = collection;
        mDialogsFactory = new ExportDialogsFactory(activity, this, this);
    ...
    public void exportApkg(String filename, Long did, boolean includeSched, boolean includeMedia) {
        ...
        exportDir.mkdirs();
        File exportPath;
        String timeStampSuffix = "-" + TimeUtils.getTimestamp(mCollection.getTime());
        if (filename != null) {
            ...
            exportPath = new File(exportDir, mCollection.getDecks().get(did).getString("name").replaceAll("\\W+", "_") + timeStampSuffix + ".apkg");
            ...
            File colPath = new File(mCollection.getPath());
