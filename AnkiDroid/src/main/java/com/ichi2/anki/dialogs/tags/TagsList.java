...
public class TagsList implements Iterable<String> {
    @NonNull
    private final TreeSet<String> mChecked;
    @NonNull
     ...
     * @param checked A list of checked tags
     *                any duplicates will be ignored
     */
    public TagsList(List<String> allTags, List<String> checked) {
        mChecked = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
        mChecked.addAll(checked);
        mAllTags = UniqueArrayList.from(allTags, String.CASE_INSENSITIVE_ORDER);
        mAllTags.addAll(mChecked);
    }
    ...
    public boolean toggleAllCheckedStatuses() {
        if (mAllTags.size() == mChecked.size()) {
            mChecked.clear();
            return true;
        }
        return mChecked.addAll(mAllTags);
    ...
    /**
     * Return all checked tags as an List that maybe not in order
     */
